...
try {
    if (isset($_POST['database'])) {
        $someVar = $_POST['database'];

        $stmtUseDatabase = $conn->prepare("use ".$someVar);
        $stmtUseDatabase->execute();
                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover">
                                        ...
                                        while ($row = $stmtTables->fetch()) {
                                            echo "<tr><td>" . $row[0] . "</td><td><form method='POST' action='description.php'><button value='" . $row[0] . "' name='table' type='submit' class='btn btn-info'>Beschreibung</button></form></td><td><form method='POST' action='content.php'><button value='" . $row[0] . "' name='table' type='submit' class='btn btn-info'>Inhalt</button></form></td></tr>";
                                        }
                                        ?>
                                        </tbody>
                                    </table>
